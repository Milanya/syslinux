...
/* Check if there are specific zeroed fields in an NTFS boot sector */
static inline int ntfs_check_zeroed_fields(const struct ntfs_boot_sector *sb)
{
    return (!sb->bsZeroed_0[0] && !sb->bsZeroed_0[1] && !sb->bsZeroed_0[2]) &&
            !sb->bsZeroed_1 && !sb->bsZeroed_2 && !sb->bsZeroed_3;
}
